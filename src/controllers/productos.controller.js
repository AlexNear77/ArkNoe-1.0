...
productsCtrl.mostrarProducto = async (req,res) =>{
   ...
   const productos = await Producto.find({"categoria": producto.categoria}).limit(3).skip(r); //Hacemos una peticion a la bd, lo filtramos segun si el usuario le pertenece y lo ordenamos por fecha de manera desendente
