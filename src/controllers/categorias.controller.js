...
categoriasCtrl.renderCategoria1 = async (req,res) =>{
   ...
   const productosA = await Producto.find({"categoria": "Equipo medico"}).limit(3);
...
categoriasCtrl.renderCategoria2 = async (req,res) =>{
   ...
   var r = Math.floor(Math.random() * n);
   const productosA = await Producto.find({"categoria": "Medio de contraste"}).limit(3);
...
categoriasCtrl.renderCategoria3 = async (req,res) =>{
   ...
   var r = Math.floor(Math.random() * n);
   const productosA = await Producto.find({"categoria": "Material radiologico"}).limit(3);
...
categoriasCtrl.renderCategoria4 = async (req,res) =>{
   ...
   var r = Math.floor(Math.random() * n);
   const productosA = await Producto.find({"categoria": "Material de curacion"}).limit(3);
